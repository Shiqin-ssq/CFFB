...
class WaveletTransformWrapper(nn.Module):
    def __init__(self, filters):
        super(WaveletTransformWrapper, self).__init__()
        self.filters = filters

    def forward(self, input):
        return WaveletTransform.apply(input, self.filters)


class InverseWaveletTransformWrapper(nn.Module):
    def __init__(self, filters):
        super(InverseWaveletTransformWrapper, self).__init__()
        self.filters = filters

    def forward(self, input):
        return InverseWaveletTransform.apply(input, self.filters)
...
class WTConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1, bias=True, wt_levels=1, wt_type='db1'):
        ...
        self.wt_function = WaveletTransformWrapper(self.wt_filter)
        self.iwt_function = InverseWaveletTransformWrapper(self.iwt_filter)
