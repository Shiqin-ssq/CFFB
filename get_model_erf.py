...
def heatmap(data, camp='RdYlGn', figsize=(6, 6), dpi=150,
            crop_ratio=1.0, save_path=None, vmin=None, vmax=None):
    h, w = data.shape

    # ===== 裁剪中心区域 =====
    if crop_ratio < 1.0:
        ch, cw = int(h * crop_ratio), int(w * crop_ratio)
        start_h, end_h = h // 2 - ch // 2, h // 2 + ch // 2
        start_w, end_w = w // 2 - cw // 2, w // 2 + cw // 2
...
class yolov8_erf:
    ...
    def process(self):
        ...
        # heatmap(data, crop_ratio=0.5, save_path="result.png", vmin=0.2, vmax=1.0)
        heatmap(data, crop_ratio=0.6, save_path="result.png")

def get_params():
    params = {
        'weight': 'CTFFB+DSC3k2.pt', # 只需要指定权重即可
        'device': 'cuda:0',
        'layer': '14', # string
        'dataset': 'mydataset2/train/images',
        'num_images': 50,
        'save_path': 'result_CTFFB_DSC3k2.png'
    }
    return params
